...
echo "[INFO] Revoke all on database \"$NEWDB\" from public"
PGPASSWORD=$PGPASSWORD psql -h $PGHOST -U postgres -p $PGPORT -c "REVOKE ALL ON DATABASE \"$NEWDB\" FROM PUBLIC;"
echo "[INFO] Revoke all on schema public when connecting to database \"$NEWDB\""
PGPASSWORD=$PGPASSWORD psql -h $PGHOST -d $NEWDB -U postgres -p $PGPORT -c "REVOKE ALL ON SCHEMA PUBLIC FROM PUBLIC;"
...
echo "[INFO] Grant all on schema public when connecting to database \"$NEWDB\" to \"$NEWUSER\""
PGPASSWORD=$PGPASSWORD psql -h $PGHOST -d $NEWDB -U postgres -p $PGPORT -c "GRANT ALL ON SCHEMA PUBLIC TO \"$NEWUSER\";"
...
echo "[INFO] Grant usage on schema public when connecting on \"$NEWDB\" to \"$NEWUSER_RO\""
PGPASSWORD=$PGPASSWORD psql -h $PGHOST -d $NEWDB -U postgres -p $PGPORT -c "GRANT USAGE ON SCHEMA PUBLIC TO \"$NEWUSER_RO\";"
